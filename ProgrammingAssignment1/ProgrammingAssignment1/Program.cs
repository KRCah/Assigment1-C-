    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (input[0] != 'q')
            {
                ...
                float distancePoints = (float)Math.Sqrt((float)Math.Pow(deltaX,2) + (float)Math.Pow(deltaY,2));
                float angleX = (float)Math.Atan2(deltaY, deltaX) * (float) (180.0 / Math.PI);

                //Console.WriteLine("delta x " + deltaX + " delta Y " + deltaY +" angulo " + angleX);
                Console.WriteLine(distancePoints + " " + angleX);
